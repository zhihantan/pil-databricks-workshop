...
VISION_SYSTEM_PROMPT = (
    ...
    "STRUCTURAL damage that affects the container's integrity — not cosmetic "
    "wear. Real containers spend years at sea and almost always look weathered; "
    "surface rust and faded paint alone are normal and do NOT make a container "
    "damaged.\n\n"
    ...
    "• RUST — judge by whether it has BREACHED the steel, not by how much "
    "staining is visible:\n"
    "  – NORMAL WEAR → 'none': reddish-brown rust STREAKS running down the "
    "panels, surface oxidation, patchy discoloration, rust at the corner "
    "posts/frame, faded or flaking paint — even when widespread — as long as the "
    "steel is intact (no holes, no flaking-through, no crumbling). Weathered used "
    "containers with vertical rust streaking all over are STILL 'none'.\n"
    "  – REAL rust damage: corrosion that has eaten THROUGH the steel "
    "(rust-through holes, perforation, heavily scaling/crumbling metal, or a "
    "rusted section that has failed). Localized rust-through = minor; extensive "
    "rust-through / large corroded holes = major.\n"
    ...
    "perfectly uniform face with no dark or brown marks at all). Diagram "
    "severity is set by EXTENT: a single small blob / a few faint speckles / a "
    ...
    "well below the container = major. (These stylized diagram marks are "
    "deliberate defects and DO count — unlike real-photo surface rust, which is "
    "weathering.)\n\n"
    "SEVERITY (either modality): none = structurally sound; at most normal wear "
    "(fading, scuffs, dirt, surface staining, and surface rust/streaking on "
    "intact steel) → 'release'. minor = light but real STRUCTURAL damage (a "
    "small dent, localized rust-through, slight door skew) → 'flag for manual "
    "inspection'. major = severe or extensive damage that compromises the "
    "container (large or multiple dents, a badly caved-in/crumpled/torn/punctured "
    "wall, extensive rust-through or large corroded holes, door hanging open or "
    "far out of frame, contents spilling) → 'remove from service'. A large "
    "caved-in, crumpled, or torn section is MAJOR even if the rest looks clean. "
    "Do NOT flag a container as minor or major for ordinary weathering, faded "
    "paint, dirt, surface rust streaks, or an odd camera angle — reserve "
    "minor/major for actual deformation, breach, or misalignment.\n"
    ...
    "photo, our schematic diagram, or not inspectable; and for a photo, whether "
    "any rust has actually breached the steel or is just surface staining), then "
    "classify. Return only JSON matching the schema."
)
